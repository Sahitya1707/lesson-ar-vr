...
const createScene = async function () {
  ...
  const box = BABYLON.MeshBuilder.CreateBox("box", {
    faceUV: faceUV,
    wrap: true,
  }); // note options parameter to set different images on each side
  ...
  const sound = new BABYLON.Sound(
    "birds",
    "./media/chirping-birds-ambience-217410.mp3",
    scene,
    null,
    {
      loop: true,
  ...
  const tree = BABYLON.SceneLoader.ImportMeshAsync(
    "",
    "./meshes/",
    "Lowpoly_tree_sample.glb"
  ).then((result) => {
  ...
  carMat.diffuseColor = new BABYLON.Color3(1, 0, 2);
  car.material = carMat;
  ...
  const wheel1 = BABYLON.SceneLoader.ImportMeshAsync(
    "",
    "./meshes/",
    "wheel1.glb"
  )
    .then((result) => {
      ...
      //   wheelMesh.rotation.y =
    })
    .catch((err) => {
      console.log("Error loading mesh" + err);
    });
